    ...
    namespace Rendering {
        namespace Rasterizing {
            namespace Shaders {
                ...
                bool PhongMeshFragmentShader::operator()(const IMeshVarying &varying, const Math::Vector4D& fragCoord, Math::Vector4D &color,
                                                         double &depth) const {
                    Math::Vector3D rgb;
                    ...
                    color = (varying.normal.dot(-(varying.light)) * rgb).componentWiseProduct(varying.intensity).toHomogeneousCoordinates(1);
